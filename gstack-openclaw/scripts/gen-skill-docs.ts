...
const HOST: Host = (() => {
  if (!HOST_ARG) return 'openclaw';
  const val = HOST_ARG.includes('=') ? HOST_ARG.split('=')[1] : process.argv[process.argv.indexOf(HOST_ARG) + 1];
  if (val === 'openclaw') return 'openclaw';
  throw new Error(`Unknown host: ${val}. Use openclaw.`);
})();
...
function generatePreambleBash(ctx: TemplateContext): string {
  const runtimeRoot = `_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
...
function transformFrontmatter(content: string, host: Host): string {
  // Find frontmatter boundaries with LF or CRLF endings.
...
function processTemplate(tmplPath: string, host: Host = 'openclaw'): { outputPath: string; content: string } {
  ...
  const openclawName = codexSkillName(skillDir === '.' ? '' : skillDir);
  const outputDir = path.join(ROOT, '.openclaw', 'skills', openclawName);
  ...
  content = content.replace(/\.claude\/skills\/review/g, '$GSTACK_ROOT/review');
  content = content.replace(/\.claude\/skills/g, 'skills');
  content = content.replace(/~\/\.codex\/skills\/gstack/g, ctx.paths.skillRoot);
  content = content.replace(/\.agents\/skills\/gstack\/review/g, '$GSTACK_ROOT/review');
  content = content.replace(/\.agents\/skills\/gstack/g, ctx.paths.localSkillRoot);
  content = content.replace(/\.agents\/skills/g, 'skills');
  content = content.replaceAll('CLAUDE.md', 'AGENTS.md');
  content = content.replace(/Claude Code/g, 'OpenClaw');
  content = content.replace(/Claude subagent/g, 'OpenClaw subagent');
  content = content.replace(/Claude main/g, 'primary agent');
  content = content.replace(/Claude structured review/g, 'primary structured review');
  content = content.replace(/Claude structured/g, 'primary structured');
  content = content.replace(/Claude adversarial/g, 'OpenClaw adversarial');
  content = content.replace(/^.*codex.*(?:\r?\n|$)/gim, '');
  content = content.replace(/^.*Codex.*(?:\r?\n|$)/gm, '');
  content = content.replace(/^.*CODEX.*(?:\r?\n|$)/gm, '');
  content = content.replace(/SOURCE:\s*"codex"\s*if\s*Codex\s*ran,\s*"claude"\s*if\s*subagent\s*ran\.\s*If\s*both\s*failed,\s*do\s*NOT\s*persist\./g, 'SOURCE: "subagent" if the secondary reviewer ran. If no secondary reviewer ran, do not persist.');
  content = content.replace(/"sessions":\s*\{\s*"claude_code":\s*(\d+),\s*"codex":\s*\d+,\s*"gemini":\s*\d+\s*\}/g, '"sessions": { "openclaw": $1 }');
