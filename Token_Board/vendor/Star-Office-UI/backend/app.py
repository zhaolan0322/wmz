...
TOKEN_BOARD_ROOT = os.path.dirname(os.path.dirname(ROOT_DIR))
...
OFFICE_AGENT_META = {
    "mishu": {"label": "秘书", "office_name": "秘书龙虾办公室"},
    "wangzai": {"label": "旺仔", "office_name": "旺仔龙虾办公室"},
    "jinbao": {"label": "金宝", "office_name": "金宝龙虾办公室"},
    "houqin": {"label": "后勤", "office_name": "后勤龙虾办公室"},
    "zhaocaixia": {"label": "昭财虾", "office_name": "昭财虾办公室"},
}
OFFICE_AGENT_IDS = frozenset(OFFICE_AGENT_META.keys())

if TOKEN_BOARD_ROOT not in sys.path:
    sys.path.insert(0, TOKEN_BOARD_ROOT)

try:
    from sync_mishu_star_office import (
        latest_assistant_reply as _sync_latest_assistant_reply,
        latest_record as _sync_latest_record,
        map_state as _sync_map_state,
        state_from_reply as _sync_state_from_reply,
    )
except Exception:
    _sync_latest_assistant_reply = None
    _sync_latest_record = None
    _sync_map_state = None
    _sync_state_from_reply = None
...
def get_effective_office_name():
    workspace_name = os.path.basename(os.path.normpath(OPENCLAW_WORKSPACE or "")).lower()
    fallback_secretary_name = "秘书龙虾办公室" if workspace_name == "mishu" else None

    office_name = get_office_name_from_identity()
    if office_name:
        if office_name in {"秘书的办公室", "秘书辦公室"} or workspace_name == "mishu":
            return "秘书龙虾办公室"
        return office_name

    if not os.path.isfile(IDENTITY_FILE):
        return fallback_secretary_name

    try:
        with open(IDENTITY_FILE, "r", encoding="utf-8") as f:
            content = f.read()
        candidates = [
            re.search(r"bot_name:\s*(.+)", content),
            re.search(r"-\s*中文名[:：]\s*(.+)", content),
        ]
        for match in candidates:
            if not match:
                continue
            name = match.group(1).strip().replace("\r", "").split("\n")[0].strip()
            if not name:
                continue
            if name == "秘书" or workspace_name == "mishu":
                return "秘书龙虾办公室"
            return f"{name}的办公室"
    except Exception:
        pass
...
def get_status_office_name():
    workspace_name = os.path.basename(os.path.normpath(OPENCLAW_WORKSPACE or "")).lower()
    if workspace_name == "mishu":
        return "秘书龙虾办公室"
...
def get_office_agent_meta(agent_id: str):
    return OFFICE_AGENT_META.get((agent_id or "").strip().lower()) or OFFICE_AGENT_META["mishu"]


def personalize_agent_text(text: str, agent_id: str) -> str:
    meta = get_office_agent_meta(agent_id)
    label = meta["label"]
    return str(text or "").replace("秘书", label)


def build_live_agent_status(agent_id: str) -> dict:
    normalized_agent_id = (agent_id or "mishu").strip().lower()
    meta = get_office_agent_meta(normalized_agent_id)

    if not (_sync_latest_record and _sync_map_state and _sync_state_from_reply and _sync_latest_assistant_reply):
        return {
            "state": "idle",
            "detail": f"{meta['label']}状态暂不可用",
            "ticker_text": "",
            "progress": 0,
            "ttl_seconds": 300,
            "updated_at": datetime.now().isoformat(),
    ...
    record = _sync_latest_record(normalized_agent_id)
    fallback_state, fallback_detail = _sync_map_state(record)
    fallback_detail = personalize_agent_text(fallback_detail, normalized_agent_id)
    reply_text = _sync_latest_assistant_reply(record)
    state, detail = _sync_state_from_reply(reply_text, fallback_state, fallback_detail)
    detail = personalize_agent_text(detail, normalized_agent_id)
    updated_at = getattr(record, "last_active_at", None)

    return {
        "state": state,
        "detail": detail,
        "ticker_text": reply_text,
        "progress": 0,
        "ttl_seconds": 300,
...
def get_status():
    ...
    agent_id = (request.args.get("agentId") or "mishu").strip().lower()
    if agent_id in OFFICE_AGENT_IDS:
        return jsonify(build_live_agent_status(agent_id))

    state = load_state()
    office_name = get_status_office_name()
    if office_name:
